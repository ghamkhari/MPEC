...
a_S_Resiptory=Fastvar(length(S),1,length(Time_Set),'generation');
...
%Binary_Table=[R_Phi(C<Infinite_Capacity,:);R_Psi(C<Infinite_Capacity,:);R_Sigma;R_Delta;R_Zeta;R_Xi];
Binary_Table=Reshaping(R_Phi(R_Phi_Trusted==.5),R_Psi(R_Psi_Trusted==.5),R_Sigma(R_Sigma_Trusted==.5),R_Delta(R_Delta_Trusted==.5),R_Zeta(R_Zeta_Trusted==.5),R_Xi(R_Xi_Trusted==.5));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Set of equalities for Partial Mixed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  integer programming. 
Equl=R_Phi(R_Phi_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Psi(R_Psi_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Sigma(R_Sigma_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Delta(R_Delta_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Zeta(R_Zeta_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Xi(R_Xi_Trusted(:,K,T)==1,K,T)-1;
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;


Equl=R_Phi(R_Phi_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Psi(R_Psi_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Sigma(R_Sigma_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Delta(R_Delta_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Zeta(R_Zeta_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;
Equl=R_Xi(R_Xi_Trusted(:,K,T)==0,K,T);
if ~isempty(Equl), Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector]; end;



R_Phi(R_Phi_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));
R_Psi(R_Psi_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));
R_Sigma(R_Sigma_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));
R_Delta(R_Delta_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));
R_Zeta(R_Zeta_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));
R_Xi(R_Xi_Trusted(:,K,T)==1,K,T)=Fastvar(ones(1,1));


R_Phi(R_Phi_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
R_Psi(R_Psi_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
R_Sigma(R_Sigma_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
R_Delta(R_Delta_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
R_Zeta(R_Zeta_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
R_Xi(R_Xi_Trusted(:,K,T)==0,K,T)=Fastvar(zeros(1,1));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:Node_Number      
   ...
   %%%%% (5b)
    ...
    %%%%%% (8)   %%%%% a is defined per time slot, so no need to indexed by
    %%%%%% time
    Equl=a(:,K,T)-Lambda(G,K,T)-Sigma(:,K,T)+Delta(:,K,T);
    Equl_Coeff_Set=[Equl_Coeff_Set; Equl.ShowVector];

    
    %%%%%%%%% (9) b is defined per time slot, so no need to indexed by
    %%%%%% time
...
% %%%%%%%%%%%%%%%% This additional constraint is for self Schedule bidding
% Ineq=P_S_Max_Cap-P_S_Max;
% Ineq_Coeff_Set = [Ineq_Coeff_Set; Ineq.ShowVector];
% %%%%%%%%%%%%%%%
...
Diagnostics=optimize(Constraints, -Prob.c*Optimization_Vector, Settings);

Time_Solver_Partial_Instance=toc;
Res.sol.int.xx=value(Optimization_Vector);

if Diagnostics.problem==0
Res.sol.int.prosta='PRIMAL_FEASIBLE';
else
Res.sol.int.prosta='INFEASIBLE';    
end

Optimal_Objective_Partial=Prob.c*Res.sol.int.xx;   
Partial_Recovery;
