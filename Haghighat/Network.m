...
%%%%%%%%%%%%%%%%
    ...
    Ineq=[];
    Ineq=[Ineq; Sigma.*(P_G-P_G_Min)];
    Ineq=[Ineq; (Large-Sigma).*(P_G-P_G_Min)];
    Ineq=[Ineq; Sigma.*(P_G_Max-P_G)];
    Ineq=[Ineq; (Large-Sigma).*(P_G_Max-P_G)];
    
    Ineq=[Ineq; Delta.*(P_G_Max-P_G)];
    Ineq=[Ineq; (Large-Delta).*(P_G_Max-P_G)];
    Ineq=[Ineq; Delta.*(P_G-P_G_Min)];
    Ineq=[Ineq; (Large-Delta).*(P_G-P_G_Min)];
    
    Ineq=[Ineq; Zeta.*(P_D-P_D_Min)];
    Ineq=[Ineq; (Large-Zeta).*(P_D-P_D_Min)];
    Ineq=[Ineq; (Zeta).*(P_D_Max-P_D)];
    Ineq=[Ineq; (Large-Zeta).*(P_D_Max-P_D)];
    
    Ineq=[Ineq; Xi.*(P_D_Max-P_D)];
    Ineq=[Ineq; (Large-Xi).*(P_D_Max-P_D)];
    Ineq=[Ineq; Xi.*(P_D-P_D_Min)];
    Ineq=[Ineq; (Large-Xi).*(P_D-P_D_Min)];
    
    Ineq=[Ineq; Phi(C<Infinite_Capacity).*((C(C<Infinite_Capacity,:)+U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; Phi(C<Infinite_Capacity).*((C(C<Infinite_Capacity,:)-U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; (Large-Phi(C<Infinite_Capacity)).*((C(C<Infinite_Capacity,:)+U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; (Large-Phi(C<Infinite_Capacity)).*((C(C<Infinite_Capacity,:)-U(C<Infinite_Capacity,:)*Theta))];
    
    Ineq=[Ineq; Psi(C<Infinite_Capacity).*((C(C<Infinite_Capacity,:)+U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; Psi(C<Infinite_Capacity).*((C(C<Infinite_Capacity,:)-U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; (Large-Psi(C<Infinite_Capacity)).*((C(C<Infinite_Capacity,:)+U(C<Infinite_Capacity,:)*Theta))];
    Ineq=[Ineq; (Large-Psi(C<Infinite_Capacity)).*((C(C<Infinite_Capacity,:)-U(C<Infinite_Capacity,:)*Theta))];
...
Time_Solver_Haghighat=toc;

if strcmp(Problem_Solved.info,'Unbounded objective function (MOSEK)')==1
    Optimal_Objective_Haghighat='Unbounded';
else
    Optimal_Objective_Haghighat=trace(value(Objective_Matrix))/length(Scenario_Set);
    V=value(V);
end
